...
uint256 hashGenesisBlock("0x8017b9e4c459061b3ab1885c6b547b147ff1f5ad8f2c2af78704d22f1a81db15");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // Syiling: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Syiling Signed Message:\n";
...
{
    // Syiling: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 SEN if made of 50 SEN inputs)
            if (nBytes < 26000)
                nMinFee = 0;
        }
        else
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 21000); // Syiling: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 403200; // Syiling: 3.5 days
static const int64 nTargetSpacing = 200; // Syiling: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Syiling: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Syiling: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Syiling: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2278df6f1ecc4482f315f0daa07ffb16a4809a48afdf9a15135d7274d7770553");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Syiling Melihat Perlombongan Akhir";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("1d0046239ccb662215c25e6064df52fe15672d92f92f52f1020173a1aef0e83a8efb22f374217688c908ee5a8d16407a4a7a9adceb3786aa2ea9e2e65175a55e43") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508403638;
        block.nBits    = 504365644;
        block.nNonce   = 3143610491;

        if (fTestNet)
        {
            block.nTime    = 1508403638;
            block.nNonce   = 1466142722;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc3385a695f46203928aa481de7419de11bf4ff1f6678feadf19b48e6a0a6c507"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Syiling: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SyilingMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SyilingMiner : ProcessBlock, block not accepted");
    }
