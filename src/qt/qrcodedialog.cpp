...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SEN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("syiling:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SEN unit input in lnReqAmount, we generate the URI with SEN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SEN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
